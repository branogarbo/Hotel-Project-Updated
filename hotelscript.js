...
document.querySelectorAll('roomcont > *, .room').forEach(roomDiv => {
  roomDiv.style.transform = `scale(${window.innerHeight/892})`;
});
...
function flash(sel) {
  for (i = 0; i < 3; i++) {
    eval(`
      setTimeout(()=>{qs(sel).style = "border-color:yellow;"},${120*i+60});
      setTimeout(()=>{qs(sel).style = "border-color:red;"},${120*(i+1)});
    `);
  }
  setTimeout(() => {
    qs(sel).style = `border-color:transparent`
  }, 420);
}

function addel(position, content) {
  qs(position).insertAdjacentHTML('beforeend', content);
}
...
function checkin(room) {
  if (eval(room).guests.length == 4 || eval(room).guests.includes(qs(`#${room} input`).value)) {
    ...
    addel(`#${room} ol`, `<li onclick="this.remove();${room}.guests.splice(${room}.guests.indexOf(this.innerHTML),1);">${qs(`#${room} input`).value}</li>`);
...
function roomlock(room) {
  if (eval(room).guests.length != 0) {
    ...
    setTimeout(() => {
      qs(`#${room} img`).style = "animation-play-state:paused";
    }, 390);
  } else if (eval(room).locked == false) {
    eval(room).locked = true;
    qs(`#${room}`).style.backgroundColor = "#ff7b91c5";
    ...
    qs(`#${room}`).style.backgroundColor = "#bed8ffc5";
